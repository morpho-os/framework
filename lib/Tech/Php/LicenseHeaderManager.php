...
class LicenseHeaderManager {
    public function updateLicenseHeader(string $filePath, string $licenseText): void {
        ...
        if (false !== $res) {
            ...
            $startIndex = 0;
            if (count($fileLines)) {
                if (str_starts_with($fileLines[$startIndex], '#!')) {
                    $startIndex++;
                }
                if (str_starts_with($fileLines[$startIndex], '<?php')) {
                    $startIndex++;
                }
            }
    ...
    protected function isValidLicenseText(string $licenseText): bool {
        return preg_match('~\blicense\b~si', $licenseText) && preg_match('~\bmorpho\b~si', $licenseText);
    }
}
