...
use function Morpho\Base\{startsWith, any};
use PhpParser\Node;
...
/**
 * Some code taken from PhpParser
 ...
 */
class CodeGenerator extends PrettyPrinterAbstract {
    private $classNode;
    ...
    protected function pExpr_FuncCall(Expr\FuncCall $node) {
        $name = $node->name;
        if (startsWith($name->toString(), 'Morpho\\Code\\Js\\')) {
            //$node = $node->name->getLast();
            $name = new Node\Name($name->getLast(), $name->getAttributes());
        }
    ...
    protected function pStmt_Class(Stmt\Class_ $node) {
        // @TODO: Indent, $this->indentLevel
        // @TODO: Move to TreeRewriter
        $res = 'var ' . $node->name . ' = (function () {';
        if (count($node->stmts)) {
            // Non empty class
            $this->classNode = $node;
            $hasConstructor = any(function ($stmt) {
                if ($stmt instanceof Stmt\ClassMethod && (string)$stmt->name === '__construct') {
                    return true;
                }
            }, $node->stmts);
            if ($hasConstructor) {
                //$res .= "\n    function " . $node->name . "() {\n";
                $res .= $this->pStmts($node->stmts);// . "\n}";
            } else {
                $res .= "\n    function " . $node->name . "() {\n";
                $res .= $this->pStmts($node->stmts) . "\n}";
            }
            ...
            // Empty class, generate a default constructor.
            $res .= "\n    function " . $node->name . "() {\n    }";
        }
        $res .= "\n    return " . $node->name . ";\n}());";
        return $res;
    ...
    protected function pStmt_Property(Stmt\Property $node) {
        throw new \LogicException("Properties must not be present");
    ...
    protected function pStmt_ClassMethod(Stmt\ClassMethod $node) {
        if ((string)$node->name === '__construct') {
            /*
            $printProperties = function (array $nodes) {
                $propertyDeclarations = filter(function ($nodes) {
                    d($nodes);
                }, $nodes);
            };
            */
            $res = "function " . $this->classNode->name . '(' . $this->pCommaSeparated($node->params) . ')';
            $res .= null !== $node->stmts ? ' {' . $this->pStmts($node->stmts) . "\n" . '}' : ' {}';
            //$res .= $printProperties($node->stmts) . "\n}";
            return $res;
        }
        throw new NotImplementedException();
